...
--kbdcfg.layout = { { "us", "" }, { "ch-fr", "" } }
--kbdcfg.current = 2  -- us is our default layout

--------------------------------------------------------------------
-- Widget de notification de volume
--------------------------------------------------------------------
local alsawidget =
{
	channel = "Master",
	...
	notifications =
	{
		icons =
		{
			...
			"/home/valentin/.config/awesome/icons/spkr_02.png",
			-- the rest of the items correspond to intermediate volume levels - you can have as many as you want (but must be >= 1)
			"/home/valentin/.config/awesome/icons/spkr_04.png",
			"/home/valentin/.config/awesome/icons/spkr_01.png",
			"/home/valentin/.config/awesome/icons/spkr_01.png"
		},
		font = "PragmataPro 8", -- must be a monospace font for the bar to be sized consistently
		icon_size = 16,
...
vicious.register (alsawidget.bar, vicious.widgets.volume, function (widget, args)
	...
	return args[1]
end, 1, alsawidget.channel) -- relatively high update time, use of keys/mouse will force update

-------------------------------------------------------------------
-- Fin du widget volume
--------------------------------------------------------------------
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "desk","warp","mail","porn"}, s, layouts[1])
end
-- }}}
...
myawesomemenu = {
   ...
   { "edit config", "gvim /home/valentin/.config/awesome/rc.lua" },
...
mylocal ={
   ...
   { "dropbox", "dropboxd" },
   { "vpn", "/opt/cisco/vpn/bin/vpnui" },
   { "inkscape", "inkscape" },
   { "gimp", "gimp" },
   { "rhythmbox", "rhythmbox" },
   { "libroffice", "libreoffice" },
   { "catfish", "catfish" },
   { "trash", "thunar trash:///" }
}
...
mymainmenu = awful.menu({ items = { { "~", "thunar" },
                                    ...
                                    { "snownews", "xfce4-terminal -e snownews" },
                                    { "earthquake", "xfce4-terminal -e earthquake" },
                                    { "vim","gvim"},
                                    { "todo","gvim Dropbox/todo.txt"},
                                    { "apps", mylocal },
                                    { "awesome", myawesomemenu}
                                  }
                        })

mylauncher = awful.widget.launcher({ image = "/home/valentin/.config/awesome/icons/arch_10x10.png",
                                     width = 10,
                                     height = 10,
                                     menu = mymainmenu })
...
----------------------------------------------------------------------------------------
myclockimg = wibox.widget.imagebox("/home/valentin/.config/awesome/icons/clock.png")
mybatimg = wibox.widget.imagebox("/home/valentin/.config/awesome/icons/bat_full_01.png")
myvolimg = wibox.widget.imagebox("/home/valentin/.config/awesome/icons/spkr_01.png")
mydwimg = wibox.widget.imagebox("/home/valentin/.config/awesome/icons/net_down_02.png")
myupimg = wibox.widget.imagebox("/home/valentin/.config/awesome/icons/net_up_02.png")

netwidgetdown = wibox.widget.textbox()
--vicious.register(netwidgetdown, vicious.widgets.net,'${wlan0 down_kb}KB', 1)
netwidgetup = wibox.widget.textbox()
--vicious.register(netwidgetup, vicious.widgets.net,'${wlan0 up_kb}KB', 1)
batwidget = wibox.widget.textbox()
--vicious.register(batwidget, vicious.widgets.bat, "$2%", 13, "BAT1")
volwidget = wibox.widget.textbox()
--vicious.register(volwidget, vicious.widgets.volume, "$1%", 2, "Master")
timewidget = wibox.widget.textbox()
--vicious.register(timewidget, vicious.widgets.date, "le %d à %R ", 60)

separator = wibox.widget.textbox('<span color="#666666" font-family="pragmatatt"> >></span>')
...
for s = 1, screen.count() do
    ...
    -- Widgets that are alignedright_layout:add( to the right
    local right_layout = wibox.layout.fixed.horizontal()
    if s == 1 then right_layout:add(wibox.widget.systray()) end

    right_layout:add(separator)
    right_layout:add(mydwimg)
    right_layout:add(netwidgetdown)
    right_layout:add(separator)
    right_layout:add(myupimg)
    right_layout:add(netwidgetup)
    right_layout:add(separator)
    right_layout:add(myvolimg)
    right_layout:add(volwidget)
    right_layout:add(separator)
    right_layout:add(mybatimg)
    right_layout:add(batwidget)
    right_layout:add(separator)
    right_layout:add(myclockimg)
